...
locals {
  deleted_aosp_mirror_filestore_instance_output = "[DELETED AOSP_MIRROR FILESTORE INSTANCE] ${var.sdv_aosp_mirror_filestore_instance_name}"

  deleted_aosp_mirror_filestore_pv_output = "[DELETED AOSP_MIRROR FILESTORE PV] ${var.sdv_aosp_mirror_filestore_pv_name}"

  deleted_aosp_mirror_filestore_pvc_output = "[DELETED AOSP_MIRROR FILESTORE PVC] ${var.sdv_aosp_mirror_filestore_pvc_name}"
}

output "filestore_instance_name" {
  description = "Name of the created AOSP Mirror Filestore instance"
...
output "filestore_pv_name" {
  description = "Name of the created Persistent Volume for AOSP Mirror Filestore"
...
output "filestore_pvc_name" {
  description = "Name of the created Persistent Volume Claim for AOSP Mirror Filestore"
...
output "project_id" {
  description = "GCP Project ID where the AOSP Mirror is deployed"
  value       = var.sdv_aosp_mirror_project_id
}

output "location" {
  description = "Region (Location) of the AOSP Mirror Filestore instance"
