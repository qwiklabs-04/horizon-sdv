...
# Function to get stop the Workstation via gcloud
stop_workstation() {
  local workstation="$1"
  ...
  log_info "Stopping Workstation '${workstation}' on GCP using gcloud..."

  gcloud workstations stop "${workstation}"\
    --config="${workstation_config}"\
    --cluster="${workstation_cluster}"\
    --region="${workstation_region}"\
  || log_error "Failed stopping Workstation '${workstation}' on GCP using gcloud..."
}
...
print_header "CLOUD WORKSTATION: STOP WORKSTATION"
...
# Prevent stopping a non-existent input Workstation by checking it among existing workstations for the user
...
if [[ "${current_workstation_state}" == "STATE_STOPPED" ]]; then
  log_warning "The workstation '${input_workstation_name}' is already in STOPPED state."
else
  # Stop workstation
  stop_workstation "$input_workstation_name" "$ws_config_name" "$ws_cluster_name" "$ws_region"
  log_success "Workstation '${input_workstation_name}' has now STOPPED!"
fi
