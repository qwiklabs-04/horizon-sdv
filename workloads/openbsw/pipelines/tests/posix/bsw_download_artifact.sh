...
function bsw_download_artifacts() {
    mkdir -p "${HOME}"/bsw_posix_artifacts

    case "${OPENBSW_DOWNLOAD_URL}" in
        gs://*)
            echo "Copying artifacts from ${OPENBSW_DOWNLOAD_URL}"
            gcloud storage cp -r "${OPENBSW_DOWNLOAD_URL}"/* "${HOME}"/bsw_posix_artifacts || true
            ;;
        *)
    ...
    # Binaries require executable
    chmod -R 755 "${HOME}"/bsw_posix_artifacts
}
